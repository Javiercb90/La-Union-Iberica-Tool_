...
//	},
	...
	parseMessages_pr0game: function(doc) {
	...
	submit_pr0game_reports_data: function(doc, reportHTML, messageId) {
        // reset status window content from any previous transmission
        try {
            //galaxytoolbar.GTPlugin_general.clear_status(doc);
			
			var language = galaxytoolbar.GTPlugin_general.get_language(doc);			
			var tmp = galaxytoolbar.GTPlugin_messages.extract_pr0game_report(doc, reportHTML, language, messageId);
	...
	extract_pr0game_report: function(doc, reportHTML,language, messageId) {
		var general = galaxytoolbar.GTPlugin_general;
		...
		var moon = false;
		try {
			msg_id = (!messageId) ? parseInt(reportHTML.id.replace("message_",""), 10) : messageId;
		} catch(e) {
			return false;
		}
		//this is the only language dependent thing
		var planetname = "";
		var playername = "";
		//TODO los saco de spyRaportHead
		var cabecera = reportHTML.querySelector("div.message-content > div.spyRaport > div.spyRaportHead").innerHTML.trim();
		
		planetname = cabecera.match(/planeta\s+([^\(]+)\s*\(/)?.[1].trim() || "";
		playername = cabecera.match(/\(([^)]+)\)/)?.[1].trim() || "";
		
		//JRR
		var elementos;
		//var date = reportHTML.querySelector("div.message-details > div.message-date").innerHTML.trim(); // del tipo 22. Nov 2025, 15:47:38
		...
		var date;
		try {
			tmp = galaxytoolbar.GTPlugin_general.parseFechaReporte(reportHTML.querySelector("div.message-details > div.message-date").innerHTML.trim()); // del tipo 22. Nov
			date = this.formatearFecha(tmp);
		...
		var coordenadas = reportHTML.querySelector("div.message-content > div.spyRaport > div.spyRaportHead").getAttribute("coords"); //del tipo 1:338:12
		var coords_exp = /(\d+):(\d+):(\d+)/;
		coords = coords_exp.exec(coordenadas);
		galaxy = coords[1];
		system = coords[2];
		planet = coords[3];	
			...
			//bucle en elementos
			//tipoespiado = elemto[i].querySelector("div.spyRaportContainerHead").innerHTML.trim();
			// bucle en elemto[i].querySelector("div.spyRaportContainerRow")
		if (reportHTML.querySelector("div.spyRaportContainerHeadClass900") !== null)
			scan_depth = 1;// Hay Recursos
		if (reportHTML.querySelector("div.spyRaportContainerHeadClass200") !== null)
			scan_depth = 2;// Hay Naves
		if (reportHTML.querySelector("div.spyRaportContainerHeadClass400") !== null)
			scan_depth = 3; // Hay Defensa
		if (reportHTML.querySelector("div.spyRaportContainerHeadClass0") !== null)
			scan_depth = 4; // Hay Edificios
		if (reportHTML.querySelector("div.spyRaportContainerHeadClass100") !== null)
			scan_depth = 5; // Hay Investigacion
		
		//la mejor de forma es hacer un bucle en todos los spyRaportContainerCell. sacar el atributo data-info, traducirlo con un diccionario de codigos y coger el innerHTML y limpiarlo
		//lo metemos en los arrays 
		var contents = new Array();
		var values = new Array();

		//elementos = reportHTML.querySelector("div.message-content > div.spyRaport > div.spyRaportContainer > div.spyRaportContainerRow > div.spyRaportContainerCell[data-info]");
		elementos = reportHTML.getElementsByClassName("spyRaportContainerCell");
		for (var i = 0; i < elementos.length; i++) {
			if (elementos[i].getAttribute("data-info") !== null){
				var codigoItem = elementos[i].getAttribute("data-info");
				var amount = elementos[i].innerHTML.trim().replace(".","");
				
				values.push(parseInt(amount, 10));
				contents.push(galaxytoolbar.GTPlugin_messages.getNameByDataInfo(codigoItem));
			}
		}
	...
	getNameByDataInfo: function(codigoItem) {
		// return the english name of the Fleet, Defence, Building or Technology based on the image ID
		switch(codigoItem) {
			//recursos
			case "900_901": return "Metal";  //Correcto
			case "900_902": return "Crystal";  //Correcto
			case "900_903": return "Deuterium";  //Correcto
			case "900_911": return "Energy";  //Correcto
			// Naves
			case "200_204": return "Light Fighter";
			case "200_205": return "Heavy Fighter";
			case "200_206": return "Cruiser";
			case "200_207": return "Battleship";
			case "200_215": return "Battlecruiser";
			case "200_211": return "Bomber";
			case "200_213": return "Destroyer";
			case "200_214": return "Deathstar";
			case "200_202": return "Small Cargo";
			case "200_203": return "Large Cargo";
			case "200_208": return "Colony Ship";
			case "200_209": return "Recycler";
			case "200_210": return "Espionage Probe";
			case "200_201": return "";
			case "200_212": return "Solar Satellite";
			case "200_218": return "Reaper";
			case "200_219": return "Pathfinder";
			case "200_216": return "";
			case "200_217": return "";
			// Defensa
			case "400_401": return "Rocket Launcher";
			case "400_402": return "Light Laser";
			case "400_403": return "Heavy Laser";
			case "400_405": return "Ion Cannon";
			case "400_404": return "Gauss Cannon";
			case "400_406": return "Plasma Turret";
			case "400_407": return "Small Shield Dome";
			case "400_408": return "Large Shield Dome";
			case "400_502": return "Anti-Ballistic Missiles";
			case "400_503": return "Interplanetary Missiles";
			
			// Edificios
			case "0_1": return "Metal Mine";  //Correcto
			case "0_2": return "Crystal Mine";  //Correcto
			case "0_3": return "Deuterium Synthesizer";  //Correcto
			case "0_4": return "Solar Plant";  //Correcto
			case "0_12": return "Fusion Reactor";
			case "0_14": return "Robotics Factory";  //Correcto
			case "0_15": return "Nanite Factory";
			case "0_21": return "Shipyard";  //Correcto
			case "0_22": return "Metal Storage";  //Correcto
			case "0_23": return "Crystal Storage";  //Correcto
			case "0_24": return "Deuterium Tank";
			case "0_31": return "Research Lab";  //Correcto
			case "0_33": return "Terraformer";
			case "0_36": return "Space Dock";
			case "0_44": return "Missile Silo";
			case "0_34": return "Alliance Depot";
			case "0_41": return "Lunar Base";
			case "0_42": return "Sensor Phalanx";
			case "0_43": return "Jump Gate";
			
			//Investigacion
			case "100_113": return "Energy Technology";  //Correcto
			case "100_120": return "Laser Technology";  //Correcto
			case "100_121": return "Ion Technology";
			case "100_114": return "Hyperspace Technology";
			case "100_122": return "Plasma Technology";
			case "100_115": return "Combustion Drive";  //Correcto
			case "100_117": return "Impulse Drive";
			case "100_118": return "Hyperspace Drive";
			case "100_106": return "Espionage Technology";
			case "100_108": return "Computer Technology";
			case "100_124": return "Astrophysics";
			case "100_123": return "Intergalactic Research Network";
			case "100_199": return "Graviton Technology";
			case "100_109": return "Weapons Technology";
			case "100_110": return "Shielding Technology";  //Correcto
			case "100_111": return "Armour Technology";
			case "100_131": return "Mineral Research";
			case "100_132": return "Semi-Crystals Research";  //Correcto
			case "100_133": return "Fuel Research";
