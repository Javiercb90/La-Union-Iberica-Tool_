...
galaxytoolbar.GTPlugin_init = {
	...
	onPageDomLoadCont: function(doc) {
		...
		try {
			//only execute ingame
			if (doc.URL.search("/game/index.php") > -1 || doc.URL.search("/game.php") > -1) {
				...
				*/
				...
				if (doc.URL.search("component=galaxy") > -1 ||doc.URL.search("page=galaxy") > -1) { // /game/galaxy.php
					if (galaxytoolbar.GTPlugin_storage.target_tool_exists(doc.URL,"galaxy")) {
						...
						//element = doc.querySelector('.galaxy-grid-row > .galaxy-planet');
						//if (MutationObserver) {
				...
				if (doc.URL.search("page=messages") > -1) {
					...
					if (galaxytoolbar.GTPlugin_storage.target_tool_exists(doc.URL,"reports")) {
						galaxytoolbar.GTPlugin_messages.parseMessages_pr0game(doc);
	...
	add_send_reports_button: function(doc) {
		...
		var p = doc.getElementById("resources_mobile").firstChild;  // better placement possible within LI Tags but no longer needed with Ogame V6
