	...
	get_ogame_time: function(doc) {
		try {
			...
			if (!this.compare_ogame_version_with(doc,5,0,0))
				clock = doc.getElementsByClassName("servertime")[0];
			else
				clock = doc.getElementsByClassName("servertime")[0];
			...
			return this.parseFechaEspanol(clock.innerHTML);
		} catch(e) {
			return false;
		}
	},
	parseFechaEspanol: function(str) {
		// Diccionario de meses abreviados → número (0–11)
		const meses = {
			"ENE": 0, "FEB": 1, "MAR": 2, "ABR": 3, "MAY": 4, "JUN": 5,
			"JUL": 6, "AGO": 7, "SEP": 8, "OCT": 9, "NOV": 10, "DIC": 11
		};

		// Ejemplo: "Viernes 21 de Nov 17:33:43"
		// Quitamos el día de la semana
		const sinDia = str.replace(/^[A-Za-zÁÉÍÓÚáéíóúñÑ]+/, "").trim();

		// Extraemos partes
		const match = sinDia.match(/(\d+)\s+de\s+([A-Za-z]+)\s+(\d{2}:\d{2}:\d{2})/);
		...
		return new Date(`${año}-${mes + 1}-${dia}T${hora}`);
	},
	parseFechaReporte : function(str) {
		const meses = {
			"ENE": 0, "FEB": 1, "MAR": 2, "ABR": 3, "MAY": 4, "JUN": 5,
			"JUL": 6, "AGO": 7, "SEP": 8, "OCT": 9, "NOV": 10, "DIC": 11
		};

		// Buscar la parte de la fecha
		const m = str.match(/(\d{1,2})\.\s*([A-Za-z]+)\s*(\d{4}),\s*(\d{2}:\d{2}:\d{2})/);
		if (!m) return null;

		const dia = Number(m[1]);
		const mesAbrev = m[2].toUpperCase();
		const año = Number(m[3]);
		const hora = m[4];

		const mes = meses[mesAbrev];
		if (mes === undefined) return null;

		return new Date(año, mes, dia, ...hora.split(":").map(Number));
	},
